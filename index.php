<!doctype html>
<html>
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, height=device-height, initial-scale=1.0, user-scalable=no">
        <meta name="apple-mobile-web-app-capable" content="yes" />

        <!-- Favicon -->
        <link rel="shortcut icon" href="favicon.ico" type="image/x-icon">

        <!-- Page title -->
        <title>Profit Button</title>

        <!-- Fonts -->
        <link href="http://fonts.googleapis.com/css?family=Playfair+Display:400,700,400italic|Raleway:300,500,800|Source+Sans+Pro:300,400,600,700" rel="stylesheet" type="text/css" />
        
        
        <!-- Styles -->
        <link rel="stylesheet" href="css/slides.css" name="main-styles">

        <!-- Scripts -->
        <script src="http://code.jquery.com/jquery-1.11.3.min.js" name="jquery"></script>
        <script src="js/plugins.js" type="text/javascript" name="plugins"></script>
        <script src="js/slides.js" type="text/javascript" name="main-scripts"></script>

        <!-- Help form -->
        <link rel="stylesheet" href="css/custom.css" name="custom-styles">
        <script src="js/val_submit.js" type="text/javascript"></script>
    </head>
    <body class="slides stack  fast static">
            	

<!-- SVG Library -->
<svg xmlns="http://www.w3.org/2000/svg" style="display:none">

  <symbol id="logo-pb" viewBox="0 0 158 20">
  	<path d="M0.68,2.2 L0.68,19 L4.952,19 L4.952,14.272 L8,14.272 C12.296,14.272 14.768,11.992 14.768,8.056 C14.768,4.312 12.296,2.2 8,2.2 L0.68,2.2 Z M4.952,5.56 L7.784,5.56 C9.656,5.56 10.736,6.448 10.736,8.2 C10.736,9.976 9.656,10.912 7.784,10.912 L4.952,10.912 L4.952,5.56 Z M21.128,6.064 L16.976,6.064 L16.976,19 L21.128,19 L21.128,12.352 C21.344,10.528 22.928,9.4 25.328,9.616 L25.328,5.872 C23.48,5.896 22.016,6.736 21.128,8.248 L21.128,6.064 Z M33.224,5.92 C28.976,5.92 26.144,8.536 26.144,12.52 C26.144,16.504 28.976,19.144 33.224,19.144 C37.448,19.144 40.28,16.504 40.28,12.52 C40.28,8.536 37.448,5.92 33.224,5.92 L33.224,5.92 Z M33.224,9.208 C34.928,9.208 36.104,10.576 36.104,12.568 C36.104,14.584 34.928,15.928 33.224,15.928 C31.496,15.928 30.32,14.584 30.32,12.568 C30.32,10.576 31.496,9.208 33.224,9.208 L33.224,9.208 Z M53.696,4.552 C54.992,4.552 55.904,3.64 55.904,2.344 C55.904,1.072 54.992,0.16 53.696,0.16 C52.424,0.16 51.512,1.072 51.512,2.344 C51.512,3.64 52.424,4.552 53.696,4.552 L53.696,4.552 Z M43.112,6.52 L41.36,6.52 L41.36,9.256 L43.112,9.256 L43.112,19 L47.216,19 L47.216,9.256 L50.144,9.256 L50.144,6.52 L47.216,6.52 L47.216,5.704 C47.216,4.72 47.84,4.288 48.704,4.288 C49.232,4.288 49.88,4.48 50.432,4.792 L50.576,1.504 C49.904,1.192 48.944,1 48.056,1 C45.296,1 43.112,2.536 43.112,5.44 L43.112,6.52 Z M51.632,6.064 L51.632,19 L55.76,19 L55.76,6.064 L51.632,6.064 Z M66.608,15.424 C65.984,15.688 65.432,15.856 65,15.856 C64.232,15.88 63.8,15.448 63.8,14.44 L63.8,9.376 L67.184,9.376 L67.184,6.64 L63.8,6.64 L63.8,3.064 L59.672,3.064 L59.672,6.664 L57.872,6.664 L57.872,9.376 L59.672,9.376 L59.672,15.088 C59.672,17.776 61.472,19.192 63.968,19.192 C65.216,19.192 66.488,18.856 67.424,18.304 L66.608,15.424 Z M75.104,2.2 L75.104,19 L83.336,19 C87.296,19 89.792,17.2 89.792,14.344 C89.792,12.16 88.448,10.504 86.336,10.096 C88.112,9.592 89.216,8.176 89.216,6.4 C89.216,3.832 86.816,2.2 83.048,2.2 L75.104,2.2 Z M79.352,5.464 L82.808,5.464 C84.056,5.464 84.848,6.088 84.848,7.168 C84.848,8.248 84.056,8.92 82.808,8.92 L79.352,8.92 L79.352,5.464 Z M79.352,11.992 L82.808,11.992 C84.392,11.992 85.376,12.688 85.376,13.816 C85.376,15.016 84.392,15.712 82.808,15.712 L79.352,15.712 L79.352,11.992 Z M104.84,6.064 L100.688,6.064 L100.688,12.448 C100.688,14.2 99.752,15.496 98.168,15.52 C96.848,15.52 96.008,14.584 96.008,13.048 L96.008,6.064 L91.856,6.064 L91.856,14.224 C91.856,17.248 93.68,19.168 96.488,19.168 C98.36,19.168 99.8,18.4 100.688,16.864 L100.688,19 L104.84,19 L104.84,6.064 Z M115.688,15.424 C115.064,15.688 114.512,15.856 114.08,15.856 C113.312,15.88 112.88,15.448 112.88,14.44 L112.88,9.376 L116.264,9.376 L116.264,6.64 L112.88,6.64 L112.88,3.064 L108.752,3.064 L108.752,6.664 L106.952,6.664 L106.952,9.376 L108.752,9.376 L108.752,15.088 C108.752,17.776 110.552,19.192 113.048,19.192 C114.296,19.192 115.568,18.856 116.504,18.304 L115.688,15.424 Z M126.128,15.424 C125.504,15.688 124.952,15.856 124.52,15.856 C123.752,15.88 123.32,15.448 123.32,14.44 L123.32,9.376 L126.704,9.376 L126.704,6.64 L123.32,6.64 L123.32,3.064 L119.192,3.064 L119.192,6.664 L117.392,6.664 L117.392,9.376 L119.192,9.376 L119.192,15.088 C119.192,17.776 120.992,19.192 123.488,19.192 C124.736,19.192 126.008,18.856 126.944,18.304 L126.128,15.424 Z M134.96,5.92 C130.712,5.92 127.88,8.536 127.88,12.52 C127.88,16.504 130.712,19.144 134.96,19.144 C139.184,19.144 142.016,16.504 142.016,12.52 C142.016,8.536 139.184,5.92 134.96,5.92 L134.96,5.92 Z M134.96,9.208 C136.664,9.208 137.84,10.576 137.84,12.568 C137.84,14.584 136.664,15.928 134.96,15.928 C133.232,15.928 132.056,14.584 132.056,12.568 C132.056,10.576 133.232,9.208 134.96,9.208 L134.96,9.208 Z M152.72,5.872 C150.8,5.896 149.312,6.664 148.4,8.176 L148.4,6.064 L144.248,6.064 L144.248,19 L148.4,19 L148.4,12.16 C148.568,10.624 149.528,9.544 151.04,9.544 C152.432,9.544 153.32,10.48 153.32,11.992 L153.32,19 L157.448,19 L157.448,10.84 C157.448,7.816 155.576,5.872 152.72,5.872 L152.72,5.872 Z" id="Profit-Button" sketch:type="MSShapeGroup"></path>
  </symbol>

  <symbol id="logo" viewBox="0 0 106 31"><path d="M17.413 14.04c-.56-5.84-5.6-7-8.52-7-4.6 0-8.6 2.92-8.6 7.52 0 3 2.4 4.88 5.28 5.8 4.24 1.64 5.88 1.84 5.88 3.36 0 1.08-1.2 1.72-2.32 1.72-.28 0-2.24 0-2.52-2.04h-6.6c.6 5.84 5.68 7.36 9.04 7.36 4.92 0 9.04-2.88 9.04-7.76 0-4.8-4-5.92-7.76-6.96-1.76-.52-3.4-1.2-3.4-2.2 0-.6.48-1.48 1.88-1.48 1.96 0 2.04 1.2 2.08 1.68h6.52zm2.222 15.96h6.64v-29.6h-6.64v29.6zm9.662-24.56h6.64v-5.04h-6.64v5.04zm0 24.56h6.64v-22.2h-6.64v22.2zm32.782-29.6h-6.64v9.28c-.72-.72-2.6-2.64-6.52-2.64-5.64 0-11 4.28-11 11.8 0 6.68 4.4 11.88 11.12 11.88 4.48 0 6.08-2.2 6.72-3.12v2.4h6.32v-29.6zm-17.52 18.4c0-2.56 1.8-5.56 5.64-5.56 1.56 0 2.96.56 3.96 1.56 1 .96 1.64 2.32 1.64 3.92.08 1.64-.52 3.08-1.56 4.12s-2.52 1.68-4.12 1.68c-3.12 0-5.56-2.28-5.56-5.68v-.04zm42.502 2.4c.52-4.08-.32-7.64-3.12-10.64-2.08-2.2-5-3.52-8.4-3.52-6.76 0-11.64 5.72-11.64 11.92 0 6.6 5.4 11.76 11.76 11.76 2.28 0 4.48-.68 6.32-2 1.88-1.28 3.44-3.2 4.52-5.68h-6.8c-.8 1.16-1.92 2.08-4.04 2.08-2.6 0-4.84-1.56-5.12-3.92h16.52zm-16.44-5.04c.16-1.04 1.52-3.52 4.96-3.52s4.8 2.48 4.96 3.52h-9.92zm34.502-2.12c-.56-5.84-5.6-7-8.52-7-4.6 0-8.6 2.92-8.6 7.52 0 3 2.4 4.88 5.28 5.8 4.24 1.64 5.88 1.84 5.88 3.36 0 1.08-1.2 1.72-2.32 1.72-.28 0-2.24 0-2.52-2.04h-6.6c.6 5.84 5.68 7.36 9.04 7.36 4.92 0 9.04-2.88 9.04-7.76 0-4.8-4-5.92-7.76-6.96-1.76-.52-3.4-1.2-3.4-2.2 0-.6.48-1.48 1.88-1.48 1.96 0 2.04 1.2 2.08 1.68h6.52z" /></symbol>
  <symbol id="logo-icon" viewBox="0 0 50 41"><path d="M4,12h42c2.2,0,4,1.8,4,4v21c0,2.2-1.8,4-4,4H4c-2.2,0-4-1.8-4-4V16C0,13.8,1.8,12,4,12z"/><path opacity="0.6" d="M45.5,9h-41C3.7,9,3,8.3,3,7.5v0C3,6.7,3.7,6,4.5,6h41C46.3,6,47,6.7,47,7.5v0C47,8.3,46.3,9,45.5,9z"/><path opacity="0.4" d="M7.5,0h35C43.3,0,44,0.7,44,1.5v0C44,2.3,43.3,3,42.5,3h-35C6.7,3,6,2.3,6,1.5v0C6,0.7,6.7,0,7.5,0z"/></symbol>
  <symbol id="close" viewBox="0 0 30 30"><path d="M15 0c-8.3 0-15 6.7-15 15s6.7 15 15 15 15-6.7 15-15-6.7-15-15-15zm5.7 19.3c.4.4.4 1 0 1.4-.2.2-.4.3-.7.3s-.5-.1-.7-.3l-4.3-4.3-4.3 4.3c-.2.2-.4.3-.7.3s-.5-.1-.7-.3c-.4-.4-.4-1 0-1.4l4.3-4.3-4.3-4.3c-.4-.4-.4-1 0-1.4s1-.4 1.4 0l4.3 4.3 4.3-4.3c.4-.4 1-.4 1.4 0s.4 1 0 1.4l-4.3 4.3 4.3 4.3z"/></symbol>
  <symbol id="menu" viewBox="0 0 22 22"><path d="M1 5h20c.6 0 1-.4 1-1s-.4-1-1-1h-20c-.6 0-1 .4-1 1s.4 1 1 1zm20 5h-20c-.6 0-1 .4-1 1s.4 1 1 1h20c.6 0 1-.4 1-1s-.4-1-1-1zm0 7h-20c-.6 0-1 .4-1 1s.4 1 1 1h20c.6 0 1-.4 1-1s-.4-1-1-1z"/></symbol>
  <symbol id="share" viewBox="0 0 22 22"><path d="M21 10c-.6 0-1 .4-1 1v7h-18v-7c0-.6-.4-1-1-1s-1 .4-1 1v7c0 1.1.9 2 2 2h18c1.1 0 2-.9 2-2v-7c0-.6-.4-1-1-1zM5.5 7.5c.3 0 .5-.1.7-.3l3.8-3.8v9.6c0 .6.4 1 1 1s1-.4 1-1v-9.6l3.8 3.8c.2.2.5.3.7.3s.5-.1.7-.3c.4-.4.4-1 0-1.4l-5.5-5.5c-.1-.1-.2-.2-.3-.2-.2-.1-.5-.1-.8 0l-.3.2-5.5 5.5c-.4.4-.4 1 0 1.4.2.2.4.3.7.3z"/></symbol>
  <symbol id="facebook" viewBox="0 0 24 24"><path d="M17.4 12h-3.2v12h-4.4v-12h-2.9v-4.1h2.9v-2.4c0-3.5.9-5.5 4.8-5.5h3.2v4.1h-2c-1.5 0-1.6.6-1.6 1.7v2.1h3.6l-.4 4.1z"/></symbol>
  <symbol id="twitter" viewBox="0 1 24 23"><path d="M21.5 7.6v.6c0 6.6-5 14.1-14 14.1-2.8 0-5.4-.8-7.6-2.2l1.2.1c2.3 0 4.4-.8 6.1-2.1-2.2 0-4-1.5-4.6-3.4.3.1.6.1.9.1.5 0 .9-.1 1.3-.2-2.1-.6-3.8-2.6-3.8-5 .7.4 1.4.6 2.2.6-1.3-.9-2.2-2.4-2.2-4.1 0-.9.2-1.8.7-2.5 2.4 3 6.1 5 10.2 5.2-.1-.4-.1-.7-.1-1.1 0-2.7 2.2-5 4.9-5 1.4 0 2.7.6 3.6 1.6 1-.3 2.1-.7 3-1.3-.4 1.2-1.1 2.1-2.2 2.7 1-.1 1.9-.4 2.8-.8-.6 1.1-1.4 2-2.4 2.7z"/></symbol>
  <symbol id="dribbble" viewBox="0 0 24 24"><path d="M12 0c-6.7 0-12 5.3-12 12s5.3 12 12 12 12-5.3 12-12-5.3-12-12-12zm7.9 5.7c1.3 1.7 2.1 3.9 2.3 6.1-.4-.1-2.4-.4-4.7-.4-.8 0-1.5 0-2.3.1 0-.1-.1-.3-.3-.5l-.7-1.5c3.7-1.4 5.3-3.4 5.7-3.8zm-7.9-3.8c2.5 0 4.9.9 6.7 2.5-.3.4-1.9 2.3-5.2 3.6-1.6-2.9-3.3-5.3-3.7-5.9.6-.1 1.4-.2 2.2-.2zm-4.4 1c.4.6 2.1 3 3.7 5.8-4.4 1.2-8.2 1.2-9.2 1.2h-.1c.8-3.1 2.9-5.6 5.6-7zm-5.7 9.1v-.3h.3c1.2 0 5.6-.1 10.1-1.5l.8 1.6c-.1 0-.3 0-.4.1-5.1 1.6-7.9 6-8.3 6.7-1.6-1.7-2.5-4.1-2.5-6.6zm10.1 10.1c-2.3 0-4.4-.8-6.1-2.1.3-.5 2.4-4.4 7.9-6.3 1.3 3.6 2 6.7 2.1 7.6-1.2.6-2.6.8-3.9.8zm5.7-1.8c-.1-.8-.7-3.6-2-7.1.7-.1 1.3-.1 2-.1 2.1 0 3.7.4 4.1.5-.3 2.8-1.8 5.2-4.1 6.7z"/></symbol>
  <symbol id="pinterest" viewBox="0 0 24 24"><path d="M5.9 13.9c1.2-2-.4-2.5-.6-4-1-6.1 7.1-10.2 11.4-6 2.9 2.9 1 12-3.7 11-4.6-.9 2.2-8.1-1.4-9.5-3-1.1-4.6 3.6-3.2 5.9-.8 4-2.5 7.7-1.8 12.7 2.3-1.7 3.1-4.8 3.7-8.1 1.2.7 1.8 1.4 3.3 1.5 5.5.4 8.6-5.4 7.8-10.7-.7-4.7-5.5-7.1-10.6-6.6-4.1.4-8.1 3.7-8.3 8.3-.1 2.8.7 4.9 3.4 5.5z"/></symbol>
  <symbol id="googlePlus" viewBox="0 0 24 24"><path d="M14.9 18.1c0-2.2-1.3-3.2-2.6-4.4l-1.1-.9c-.3-.3-.8-.7-.8-1.4s.5-1.1.9-1.5c1.3-1 2.6-2.1 2.6-4.5s-1.5-3.7-2.2-4.3h2l2-1.1h-6.5c-1.7 0-3.8.3-5.6 1.7-1.4 1.2-2 2.8-2 4.2 0 2.5 1.9 4.9 5.2 4.9.3 0 .7 0 1-.1-.2.4-.3.7-.3 1.2 0 1 .5 1.6.9 2.1-1.4.1-4.1.3-6 1.4s-2.4 2.7-2.4 3.8c0 2.3 2.2 4.5 6.8 4.5 5.2.4 8.1-2.6 8.1-5.6zm-6.8-8c-2.7 0-3.9-3.5-3.9-5.6 0-.8.2-1.7.7-2.3.5-.6 1.4-1 2.2-1 2.6 0 4 3.5 4 5.8 0 .6-.1 1.6-.8 2.3-.6.4-1.4.8-2.2.8zm0 12.7c-3.4 0-5.5-1.6-5.5-3.8 0-2.2 2-3 2.7-3.2 1.3-.4 3-.5 3.3-.5h.7c2.4 1.7 3.4 2.6 3.4 4.2 0 1.8-1.6 3.3-4.6 3.3zm7-10.8h3.4v3.4h1.8v-3.4h3.4v-1.7h-3.4v-3.4h-1.8v3.4h-3.4v1.7z"/></symbol>
  <symbol id="stumbleupon" viewBox="0 0 24 24"><path d="M13.3 9.6l1.6.8 2.5-.8v-1.4c0-3-2.4-5.4-5.4-5.4s-5.4 2.4-5.4 5.4v7.5c0 .7-.6 1.3-1.3 1.3s-1.3-.6-1.3-1.3v-3.2h-4v3.2c0 3 2.4 5.4 5.4 5.4s5.4-2.4 5.4-5.4v-7.5c0-.7.6-1.3 1.3-1.3s1.3.6 1.3 1.3l-.1 1.4zm6.6 2.9v3.2c0 .7-.6 1.3-1.3 1.3s-1.3-.6-1.3-1.3v-3.2l-2.5.8-1.6-.8v3.2c0 3 2.4 5.4 5.4 5.4s5.4-2.4 5.4-5.4v-3.2h-4.1z"/></symbol>
  <symbol id="linkedin" viewBox="0 0 24 24"><path d="M22.2 0h-20.4c-1 0-1.8.8-1.8 1.7v20.7c0 1 .8 1.7 1.8 1.7h20.5c1 0 1.8-.8 1.8-1.7v-20.7c-.1-.9-.9-1.7-1.9-1.7zm-14.9 20.2h-3.6v-10.9h3.6v10.9zm-1.8-12.4c-1.2 0-2-.8-2-1.9 0-1.1.8-1.9 2.1-1.9 1.2 0 2 .8 2 1.9-.1 1.1-.9 1.9-2.1 1.9zm14.8 12.4h-3.6v-5.8c0-1.5-.5-2.5-1.8-2.5-1 0-1.6.7-1.9 1.3-.1.2-.1.6-.1.9v6.1h-3.6v-10.9h3.6v1.5c.5-.7 1.3-1.8 3.3-1.8 2.4 0 4.2 1.6 4.2 4.9v6.3z"/></symbol>
  <symbol id="apple" viewBox="-1 1 24 24"><path d="M17.6 13.8c0-3 2.5-4.5 2.6-4.6-1.4-2.1-3.6-2.3-4.4-2.4-1.9-.2-3.6 1.1-4.6 1.1-.9 0-2.4-1.1-4-1-2 0-3.9 1.2-5 3-2.1 3.7-.5 9.1 1.5 12.1 1 1.5 2.2 3.1 3.8 3 1.5-.1 2.1-1 3.9-1s2.4 1 4 1 2.7-1.5 3.7-2.9c1.2-1.7 1.6-3.3 1.7-3.4-.1-.1-3.2-1.3-3.2-4.9zm-3.1-9c.8-1 1.4-2.4 1.2-3.8-1.2 0-2.7.8-3.5 1.8-.8.9-1.5 2.3-1.3 3.7 1.4.1 2.8-.7 3.6-1.7z"/></symbol>
  <symbol id="tumblr" viewBox="0 0 23 23"><path d="M12.573 4.94v-4.94h-3.188c-.072.183-.11.4-.11.622-.034.107-.072.184-.072.293-.328 1.829-1.28 3.11-2.892 3.807-.476.218-.914.253-1.39.218v3.987h2.342c.039 5.603.039 8.493.039 8.64v.332c.294 2.449 1.573 3.914 3.843 4.463.914.257 1.901.366 2.892.366 1.279-.036 2.525-.256 3.771-.659v-4.685c-.731.22-1.395.402-1.977.583-1.135.333-2.087.113-2.857-.619-.073-.11-.183-.257-.221-.403-.106-.586-.178-1.206-.178-1.795v-6.222h5.083v-3.988h-5.085z"/></symbol>
  <symbol id="arrow-down" viewBox="0 0 24 24"><path d="M12 18c-.2 0-.5-.1-.7-.3l-11-10c-.4-.4-.4-1-.1-1.4.4-.4 1-.4 1.4-.1l10.4 9.4 10.3-9.4c.4-.4 1-.3 1.4.1.4.4.3 1-.1 1.4l-11 10c-.1.2-.4.3-.6.3z"/></symbol>
  <symbol id="arrow-up" viewBox="0 0 24 24"><path d="M11.9 5.9c.2 0 .5.1.7.3l11 10c.4.4.4 1 .1 1.4-.4.4-1 .4-1.4.1l-10.4-9.4-10.3 9.4c-.4.4-1 .3-1.4-.1-.4-.4-.3-1 .1-1.4l11-10c.1-.2.4-.3.6-.3z"/></symbol>
  <symbol id="arrow-left" viewBox="0 0 31 72"><path d="M30 72c-.3 0-.6-.1-.8-.4l-29-34c-.3-.4-.3-.9 0-1.3l29-36c.3-.4 1-.5 1.4-.2.4.3.5 1 .2 1.4l-28.5 35.5 28.5 33.4c.4.4.3 1.1-.1 1.4-.2.1-.5.2-.7.2z"/></symbol>
  <symbol id="arrow-right" viewBox="0 0 31 72"><path d="M1 0c.3 0 .6.1.8.4l29 34c.3.4.3.9 0 1.3l-29 36c-.3.4-1 .5-1.4.2-.4-.3-.5-1-.2-1.4l28.5-35.5-28.5-33.4c-.4-.4-.3-1.1.1-1.4.2-.1.5-.2.7-.2z"/></symbol>
  <symbol id="play" viewBox="0 0 30 30"><path d="M7 30v-30l22 15z"/></symbol>
  <symbol id="fb-like" viewBox="0 0 20 20"><path d="M0 8v12h5v-12h-5zm2.5 10.8c-.4 0-.8-.3-.8-.8 0-.4.3-.8.8-.8s.8.3.8.8c0 .4-.4.8-.8.8zm3.5-.8h9.5c1.1 0 1.7-1 1.7-1.7 0-.3-.4-1-.4-1 1.4-.3 1.7-1.2 1.7-1.7-.1-.5-.3-.9-.5-1 1-.4 1.5-1.1 1.4-1.9-.1-.8-1-1.5-1-1.5 1-.6.9-1.5.9-1.5-.3-1.3-1.5-1.7-1.7-1.7h-5.6s.3-.5.3-2.4-1.3-3.6-2.6-3.6c0 0-.7.1-1 .3v3.5l-2.7 4.4v9.8z"/></symbol>
  <symbol id="appstoreButton" viewBox="0 0 135 40"><path fill="#000" d="M135 36.2c0 2.1-1.7 3.8-3.8 3.8h-127.4c-2.1 0-3.8-1.7-3.8-3.8v-32.3c0-2.2 1.7-3.9 3.8-3.9h127.3c2.1 0 3.8 1.7 3.8 3.8l.1 32.4z"/><path fill="#ffffff" d="M30.1 19.8c0-3.2 2.6-4.8 2.8-4.9-1.5-2.2-3.9-2.5-4.7-2.5-2-.2-3.9 1.2-4.9 1.2s-2.6-1.2-4.2-1.1c-2.1 0-4.1 1.3-5.2 3.2-2.3 3.9-.6 9.7 1.6 12.9 1.1 1.6 2.4 3.3 4 3.2 1.6-.1 2.2-1 4.2-1 1.9 0 2.5 1 4.2 1s2.8-1.6 3.9-3.1c1.3-1.8 1.8-3.5 1.8-3.6-.1-.2-3.4-1.5-3.5-5.3zM26.9 10.3c.9-1.1 1.5-2.6 1.3-4.1-1.3.1-2.8.9-3.8 1.9-.8.9-1.5 2.5-1.3 3.9 1.5.2 2.9-.6 3.8-1.7zM53.6 31.5h-2.3l-1.2-3.9h-4.3l-1.2 3.9h-2.2l4.3-13.3h2.6l4.3 13.3zm-3.8-5.5l-1.1-3.5c-.2-.4-.4-1.2-.7-2.5-.1.6-.3 1.4-.6 2.5l-1.2 3.5h3.6zM64.7 26.6c0 1.6-.4 2.9-1.3 3.9-.8.8-1.8 1.3-2.9 1.3-1.3 0-2.2-.5-2.7-1.4v5.1h-2.1v-10.4c0-1 0-2.1-.1-3.2h1.9l.1 1.5c.7-1.1 1.8-1.7 3.2-1.7 1.1 0 2.1.4 2.8 1.3.7.9 1.1 2.1 1.1 3.6zm-2.2.1c0-.9-.2-1.7-.6-2.3-.5-.6-1.1-.9-1.9-.9-.5 0-1 .2-1.4.5-.4.3-.7.8-.8 1.4-.1.3-.1.5-.1.7v1.6c0 .7.2 1.3.6 1.8s1 .7 1.7.7c.8 0 1.4-.3 1.9-.9.4-.8.6-1.6.6-2.6zM75.7 26.6c0 1.6-.4 2.9-1.3 3.9-.8.8-1.8 1.3-2.9 1.3-1.3 0-2.2-.5-2.7-1.4v5.1h-2.1v-10.4c0-1 0-2.1-.1-3.2h1.9l.1 1.5c.7-1.1 1.8-1.7 3.2-1.7 1.1 0 2.1.4 2.8 1.3.7.9 1.1 2.1 1.1 3.6zm-2.2.1c0-.9-.2-1.7-.6-2.3-.5-.6-1.1-.9-1.9-.9-.5 0-1 .2-1.4.5-.4.3-.7.8-.8 1.4-.1.3-.1.5-.1.7v1.6c0 .7.2 1.3.6 1.8.4.3 1 .5 1.7.5.8 0 1.4-.3 1.9-.9.4-.6.6-1.4.6-2.4zM88 27.8c0 1.1-.4 2.1-1.2 2.8-.9.8-2.1 1.2-3.6 1.2-1.4 0-2.6-.3-3.4-.8l.5-1.8c.9.6 2 .8 3.1.8.8 0 1.4-.2 1.9-.5.4-.4.7-.8.7-1.5 0-.5-.2-1-.6-1.4-.4-.4-1-.7-1.8-1-2.4-.9-3.6-2.2-3.6-3.9 0-1.1.4-2 1.2-2.7.8-.7 1.9-1 3.3-1 1.2 0 2.2.2 3 .6l-.5 1.8c-.8-.4-1.6-.6-2.5-.6-.8 0-1.3.2-1.8.6-.4.3-.5.7-.5 1.2s.2 1 .6 1.3c.4.3 1 .7 1.9 1 1.1.5 2 1 2.5 1.6.6.6.8 1.4.8 2.3zM95.1 23.5h-2.3v4.7c0 1.2.4 1.8 1.2 1.8.4 0 .7 0 .9-.1l.1 1.6c-.4.2-1 .2-1.7.2-.8 0-1.5-.3-2-.8s-.7-1.4-.7-2.6v-4.8h-1.4v-1.6h1.4v-1.8l2.1-.6v2.4h2.3l.1 1.6zM105.7 26.6c0 1.5-.4 2.7-1.3 3.6-.9 1-2.1 1.5-3.5 1.5s-2.5-.5-3.4-1.4c-.8-.9-1.3-2.1-1.3-3.5 0-1.5.4-2.7 1.3-3.7.9-.9 2-1.4 3.5-1.4 1.4 0 2.5.5 3.4 1.4.9.9 1.3 2.1 1.3 3.5zm-2.2.1c0-.9-.2-1.6-.6-2.3-.4-.8-1.1-1.1-1.9-1.1-.9 0-1.5.4-2 1.1-.4.6-.6 1.4-.6 2.3s.2 1.6.6 2.3c.5.8 1.1 1.1 1.9 1.1.8 0 1.5-.4 1.9-1.2.5-.6.7-1.3.7-2.2zM112.6 23.8c-.2 0-.4-.1-.7-.1-.8 0-1.3.3-1.7.8s-.5 1.1-.5 1.9v5h-2.1v-6.6c0-1.1 0-2.1-.1-3h1.9l.1 1.8h.1c.2-.6.6-1.1 1.1-1.5.5-.3 1-.5 1.5-.5h.5v2.2h-.1zM122.2 26.3c0 .4 0 .7-.1 1h-6.4c0 .9.3 1.7.9 2.2.5.4 1.2.7 2.1.7.9 0 1.8-.2 2.6-.5l.3 1.5c-.9.4-2 .6-3.2.6-1.5 0-2.7-.4-3.5-1.3s-1.3-2.1-1.3-3.5.4-2.7 1.2-3.6c.8-1 1.9-1.5 3.4-1.5 1.4 0 2.4.5 3.1 1.5.6.6.9 1.7.9 2.9zm-2.1-.6c0-.6-.1-1.2-.4-1.6-.4-.6-.9-.9-1.7-.9-.7 0-1.3.3-1.7.9-.4.5-.6 1-.6 1.7l4.4-.1zM49 10c0 1.2-.4 2.1-1.1 2.7-.7.5-1.6.8-2.8.8-.6 0-1.1 0-1.5-.1v-6.4c.6-.1 1.2-.1 1.8-.1 1.1 0 2 .2 2.6.7.7.6 1 1.4 1 2.4zm-1.1 0c0-.8-.2-1.3-.6-1.8-.4-.4-1-.6-1.8-.6-.3 0-.6 0-.8.1v4.9h.7c.8 0 1.4-.2 1.9-.7.4-.4.6-1 .6-1.9zM54.9 11c0 .7-.2 1.3-.6 1.8s-1 .7-1.7.7-1.2-.2-1.7-.7c-.4-.5-.6-1-.6-1.7s.2-1.3.6-1.8 1-.7 1.7-.7 1.2.2 1.7.7c.4.5.6 1 .6 1.7zm-1.1.1c0-.4-.1-.8-.3-1.1-.2-.4-.5-.6-.9-.6s-.7.2-1 .6c-.2.3-.3.7-.3 1.1 0 .4.1.8.3 1.1.2.4.5.6 1 .6.4 0 .7-.2.9-.6.2-.3.3-.7.3-1.1zM62.8 8.7l-1.5 4.7h-1l-.6-2c-.2-.5-.3-1-.4-1.5-.1.5-.2 1-.4 1.5l-.6 2h-1l-1.4-4.7h1.1l.5 2.3.3 1.5c.2-.4.3-.9.5-1.5l.7-2.2h.9l.6 2.2c.2.5.3 1.1.4 1.6.1-.5.2-1 .3-1.6l.6-2.2 1-.1zM68.2 13.4h-1v-2.7c0-.8-.3-1.2-1-1.2-.3 0-.6.1-.8.3-.2.2-.3.5-.3.8v2.8h-1v-4.7h.9v.7c.1-.2.3-.4.5-.6.3-.2.6-.3.9-.3.4 0 .8.1 1.1.4.4.3.5.9.5 1.6v2.9h.2zM71.1 13.4h-1v-6.9h1v6.9zM77.3 11c0 .7-.2 1.3-.6 1.8-.4.5-1 .7-1.7.7s-1.2-.2-1.7-.7c-.4-.5-.6-1-.6-1.7s.2-1.3.6-1.8c.4-.5 1-.7 1.7-.7s1.2.2 1.7.7c.4.5.6 1 .6 1.7zm-1.1.1c0-.4-.1-.8-.3-1.1-.2-.4-.5-.6-.9-.6s-.7.2-1 .6c-.2.3-.3.7-.3 1.1 0 .4.1.8.3 1.1.2.4.5.6 1 .6.4 0 .7-.2.9-.6.2-.3.3-.7.3-1.1zM82.3 13.4h-.9l-.1-.5c-.3.4-.8.7-1.4.7-.4 0-.8-.1-1.1-.4-.2-.3-.4-.6-.4-1 0-.6.2-1 .7-1.3.5-.3 1.2-.5 2-.4v-.1c0-.6-.3-.9-1-.9-.5 0-.9.1-1.2.3l-.1-.8c.4-.3 1-.4 1.6-.4 1.2 0 1.8.6 1.8 1.9v1.7c.1.6.1 1 .1 1.2zm-1.1-1.6v-.7c-1.2 0-1.7.3-1.7.9 0 .2.1.4.2.6.1.1.3.2.5.2s.4-.1.6-.2c.2-.1.3-.3.4-.6v-.2zM88.3 13.4h-.9v-.8c-.3.6-.8.9-1.5.9-.6 0-1-.2-1.4-.7-.4-.4-.6-1-.6-1.7 0-.8.2-1.4.6-1.9.4-.4.9-.7 1.5-.7s1.1.2 1.3.6v-2.7h1v5.6c-.1.6 0 1 0 1.4zm-1.1-2v-1.1c-.1-.3-.2-.5-.4-.6-.2-.2-.4-.3-.7-.3-.4 0-.7.2-.9.5-.2.3-.3.7-.3 1.2s.1.8.3 1.1c.2.3.5.5.9.5.3 0 .6-.1.8-.4.2-.2.3-.5.3-.9zM97.2 11c0 .7-.2 1.3-.6 1.8-.4.5-1 .7-1.7.7s-1.2-.2-1.7-.7c-.4-.5-.6-1-.6-1.7s.2-1.3.6-1.8 1-.7 1.7-.7 1.2.2 1.7.7c.4.5.6 1 .6 1.7zm-1 .1c0-.4-.1-.8-.3-1.1-.2-.4-.5-.6-.9-.6s-.7.2-1 .6c-.2.3-.3.7-.3 1.1 0 .4.1.8.3 1.1.2.4.5.6 1 .6.4 0 .7-.2.9-.6.2-.3.3-.7.3-1.1zM102.9 13.4h-1v-2.7c0-.8-.3-1.2-1-1.2-.3 0-.6.1-.8.3s-.3.5-.3.8v2.8h-1v-4.7h.9v.7c.1-.2.3-.4.5-.6.3-.2.6-.3 1-.3s.8.1 1.1.4c.4.3.5.9.5 1.6v2.9h.1zM109.9 9.5h-1.2v2.3c0 .6.2.9.6.9h.5v.8c-.2.1-.5.1-.8.1-.4 0-.7-.1-1-.4-.2-.3-.3-.7-.3-1.3v-2.4h-.7v-.8h.7v-.9l1-.3v1.2h1.2v.8zM115.5 13.4h-1v-2.7c0-.8-.3-1.3-.9-1.3-.5 0-.8.2-1 .7v3.2h-1v-6.9h1v2.8c.3-.5.8-.8 1.4-.8.4 0 .8.1 1.1.4.4.4.5.9.5 1.6v3h-.1zM121.2 10.9v.5h-3.2c0 .5.2.8.5 1.1.3.2.6.3 1 .3.5 0 .9-.1 1.3-.2l.2.7c-.4.2-1 .3-1.6.3-.7 0-1.3-.2-1.7-.6s-.6-1-.6-1.7.2-1.3.6-1.8c.4-.5 1-.8 1.6-.8.7 0 1.2.3 1.5.8.3.3.4.8.4 1.4zm-1-.3c0-.3-.1-.6-.2-.8-.2-.3-.5-.4-.8-.4s-.6.1-.8.4c-.2.2-.3.5-.3.8h2.1z"/></symbol>
  <symbol id="login" viewBox="0 0 22 22"><path d="M13 2c0 .6.4 1 1 1h6v16h-6c-.6 0-1 .4-1 1s.4 1 1 1h6c1.1 0 2-.9 2-2v-16c0-1.1-.9-2-2-2h-6c-.6 0-1 .4-1 1zm-6.5 3.5c0 .3.1.5.3.7l3.8 3.8h-9.6c-.6 0-1 .4-1 1s.4 1 1 1h9.6l-3.8 3.8c-.2.2-.3.5-.3.7s.1.5.3.7c.4.4 1 .4 1.4 0l5.5-5.5c.1-.1.2-.2.2-.3.1-.2.1-.5 0-.8l-.2-.3-5.5-5.5c-.4-.4-1-.4-1.4 0-.2.2-.3.4-.3.7z"/></symbol>
  <symbol id="drop-down" viewBox="0 0 16 16"><polyline stroke-width="2" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" points="1,5 8,12 15,5" fill="none"/></symbol>
  <symbol id="direction-horizontal" viewBox="-2 6 16 16"><path d="M13.7 13.3l-5-5c-.4-.4-1-.4-1.4 0s-.4 1 0 1.4l3.3 3.3h-11.6c-.6 0-1 .4-1 1s.4 1 1 1h11.6l-3.3 3.3c-.4.4-.4 1 0 1.4.2.2.4.3.7.3s.5-.1.7-.3l5-5c.2-.2.3-.5.3-.7s-.1-.5-.3-.7z"/></symbol>
  <symbol id="direction-vertical" viewBox="-2 6 16 16"><path d="M6.7 21.7l5-5c.4-.4.4-1 0-1.4s-1-.4-1.4 0l-3.3 3.3v-11.6c0-.6-.4-1-1-1s-1 .4-1 1v11.6l-3.3-3.3c-.4-.4-1-.4-1.4 0-.2.2-.3.4-.3.7 0 .3.1.5.3.7l5 5c.2.2.4.3.7.3s.5-.1.7-.3z"/></symbol>
</svg>

<!-- Navigation -->
<nav class="side  medium ">
  <div class="navigation">
    <ul>
    </ul>
  </div>
</nav>

<!-- Panel top #01 -->
<nav class="panel top forceMobileView">
  <div class="sections desktop">
    <div class="left"><a href="index.php"><svg style="width:180px;height:50px"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#logo-pb"></use></svg></a></div>
    <div class="center">
      <ul class="menu">
        <li><a href="#overview">Overview</a></li>
        <li><a href="#features">Features</a></li>
        <li><a href="#app">App</a></li>
        <li><a href="#try">Try</a></li>
        <li><a href="#video">Video</a></li>
        <li><a href="#presentation">Presentation</a></li>
        <li><a href="#hello">Hello</a></li>
        <li><a href="#contacts">Contacts</a></li>
        <li><a href="advisers.html"><b>Advisers</b></a></li>
      </ul>
    </div>
    <div class="right"> <a class="button menuButton" href="http://admin.probtn.com" target="_blank">Enter</a></div>
  </div>
  <div class="sections compact hidden">
    <div class="left"><a href="index.php"><svg style="width:180px;height:50px"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#logo-pb"></use></svg></a></div>
    <div class="right"><span class="button actionButton sidebarTrigger" data-sidebar-id="1"><svg><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#menu"></use></svg></span></div>
  </div>
</nav>

<!-- Sidebar -->
<nav class="sidebar blue" data-sidebar-id="1">
  <div class="close"><svg><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#close"></use></svg></div>
  <div class="content">
    <a href="index.php" class="logo"><svg width="180" height="50"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#logo-pb"></use></svg></a>
    <ul class="subMenu">
        <li><a href="#overview">Overview</a></li>
        <li><a href="#features">Features</a></li>
        <li><a href="#app">App</a></li>
        <li><a href="#try">Try</a></li>
        <li><a href="#video">Video</a></li>
        <li><a href="#presentation">Presentation</a></li>
        <li><a href="#hello">Hello</a></li>
        <li><a href="#contacts">Contacts</a></li>
        <li><a href="advisers.html"><b>Advisers</b></a></li>
    </ul>
    <ul class="social">
      <li><a target="_blank" href="https://www.facebook.com/pizzabtn"><svg><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#facebook"></use></svg></a></li>
      <li><a target="_blank" href="https://twitter.com/probtn"><svg><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#twitter"></use></svg></a></li>
    </ul>
  </div>
</nav>

<!-- Slide #1 — OVERVIEW -->
<section class="slide fade kenBurns" data-title="Overview" name="overview">
  <div class="content">
    <div class="container">
      <div class="wrap top">
      
        <div class="fix-10-12 toCenter">
          <h1 class="small ae-3">Floating buttons for mobile web and applications</h1>
          <div class="ae-4"><p>New advertising format. Additional layer on top of the page. 
			Easy to move with a thumb. Unfold into the full screen with one&nbsp;click.</p></div>
        </div>
        <div class="fix-10-12 ae-5" data-action="zoom">
          <img class="iphone-35" src="assets/img/iphone-1@2x.png" alt="Profit Button"/>
        </div>
        
      </div>
    </div>
  </div>
  <div class="background" style="background-image:url(assets/img/img-1.jpg)"></div>
</section>

<!-- Slide #2 — FEATURES -->
<section class="slide fade kenBurns" data-title="Features" name="features">
  <div class="content">
    <div class="container">
      <div class="wrap">
      
        <div class="fix-10-12 toCenter">
          <h1 class="small ae-1">All about floating buttons</h1>
        </div>
        <div class="fix-12-12">
          <ul class="grid later equal">
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-3">
                <!-- <svg width="72" height="72" viewBox="0 0 72 68" xmlns="http://www.w3.org/2000/svg" class="icon-47"><g stroke="#fff" stroke-width="3" stroke-linejoin="round" fill="none"><path d="M70.5 30.276c0 15.543-15.444 28.149-34.5 28.149-4.254 0-8.325-.63-12.087-1.779l-19.827 9.228 7.764-15.498c-6.39-5.106-10.35-12.228-10.35-20.1 0-15.546 15.444-28.149 34.5-28.149 19.056 0 34.5 12.603 34.5 28.149z"/><path d="M22.5 30.628c0 2.481-2.016 4.5-4.5 4.5s-4.5-2.019-4.5-4.5c0-2.487 2.016-4.5 4.5-4.5s4.5 2.013 4.5 4.5z"/><path d="M40.5 30.628c0 2.481-2.016 4.5-4.5 4.5s-4.5-2.019-4.5-4.5c0-2.487 2.016-4.5 4.5-4.5s4.5 2.013 4.5 4.5z"/><path d="M58.5 30.628c0 2.481-2.016 4.5-4.5 4.5s-4.5-2.019-4.5-4.5c0-2.487 2.016-4.5 4.5-4.5s4.5 2.013 4.5 4.5z"/></g></svg> -->
                <img src="assets/img/features/icon-unique@2x.png" width="220px" height="161px">
                <h3 class="uppercase">UNIQUE</h3>
                <p class="small">A new way of interaction between a user and their device that let them add new features without changing the design and the code of an app.</p>
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-4">
                <!-- <svg width="72" height="72" viewBox="0 0 73 72" xmlns="http://www.w3.org/2000/svg" class="icon-47"><g stroke="#fff" stroke-width="3" stroke-linejoin="round" fill="none"><path d="M70.538 36c0 19.053-15.447 34.5-34.5 34.5-19.056 0-34.5-15.447-34.5-34.5 0-19.056 15.444-34.5 34.5-34.5 19.053 0 34.5 15.444 34.5 34.5z"/><path d="M58.538 39c0 12.429-10.074 22.5-22.5 22.5s-22.5-10.071-22.5-22.5h45z"/><path d="M16.538 30c0-3.312 2.688-6 6-6s6 2.688 6 6"/><path d="M55.538 30c0-3.312-2.688-6-6-6s-6 2.688-6 6"/></g></svg> -->
                <img src="assets/img/features/icon-speed@2x.png" width="220px" height="161px">
                <h3 class="uppercase">SPEED</h3>
                <p class="small">You build in the button code once. Then you can change a design and the content via our user-friendly administration platform.</p>
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-5">
                <!-- <svg width="72" height="72" viewBox="0 0 72 72" xmlns="http://www.w3.org/2000/svg" class="icon-47"><g stroke="#fff" stroke-width="3" stroke-linejoin="round" fill="none"><path d="M70.5 36c0 19.053-15.444 34.5-34.5 34.5-19.056 0-34.5-15.447-34.5-34.5 0-19.056 15.444-34.5 34.5-34.5 19.056 0 34.5 15.444 34.5 34.5z"/><path d="M36 54s16.5-12 16.5-20.886c0-10.257-13.5-13.5-16.5-1.023-3-12.477-16.5-9.234-16.5 1.023 0 8.886 16.5 20.886 16.5 20.886z"/></g></svg> -->
                <img src="assets/img/features/icon-conversion@2x.png" width="220px" height="161px">
                <h3 class="uppercase">CONVERSION</h3>
                <p class="small">Our customers are surprised with the button’s metrics that are a lot more interesting than traditional mobile ads.</p>
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-6">
                <!-- <svg width="72" height="72" viewBox="0 0 72 42" xmlns="http://www.w3.org/2000/svg" class="icon-47"><g stroke="#fff" stroke-width="3" stroke-linejoin="round" fill="none"><path d="M46.5 37.5c0 1.65-1.35 3-3 3h-39c-1.65 0-3-1.35-3-3v-33c0-1.647 1.35-3 3-3h39c1.65 0 3 1.353 3 3v33z"/><path d="M70.5 37.5l-18-9v-15l18-9v33z"/></g></svg> -->
                <img src="assets/img/features/icon-economy@2x.png" width="220px" height="161px">
                <h3 class="uppercase">ECONOMY</h3>
                <p class="small">It is less costly and less time consuming to add new functions while using floating buttons.</p>
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-7">
                <!-- <svg  width="72" height="72" viewBox="0 0 73 72" xmlns="http://www.w3.org/2000/svg" class="icon-47"><g fill="none" stroke="#fff" stroke-width="3" stroke-linejoin="round"><path d="M43.538 36c0 4.143-3.36 7.5-7.5 7.5-4.143 0-7.5-3.357-7.5-7.5s3.357-7.5 7.5-7.5c4.14 0 7.5 3.357 7.5 7.5z"/><path d="M46.538 25.5h7.5c2.475 0 4.5 2.025 4.5 4.5v15c0 2.475-2.025 4.5-4.5 4.5h-36c-2.475 0-4.5-2.025-4.5-4.5v-15c0-2.475 2.025-4.5 4.5-4.5h7.5M25.538 25.5l6-6h9l6 6" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/><path d="M52.538 28.5c-.828 0-1.5.672-1.5 1.5s.672 1.5 1.5 1.5 1.5-.672 1.5-1.5-.672-1.5-1.5-1.5"/><path d="M70.538 36c0 19.056-15.447 34.5-34.5 34.5-19.056 0-34.5-15.444-34.5-34.5 0-19.056 15.444-34.5 34.5-34.5 19.053 0 34.5 15.444 34.5 34.5z"/></g></svg> -->
                <img src="assets/img/features/icon-platform@2x.png" width="220px" height="161px">
                <h3 class="uppercase">ANY PLATFORM</h3>
                <p class="small">Floating button is available for iOS, Android, Windows Phone, Web and Mobile Web.</p>
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-8">
                <!-- <svg width="72" height="72" viewBox="0 0 71 71" xmlns="http://www.w3.org/2000/svg" class="icon-47"><g stroke="#fff" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" fill="none"><path d="M48.782 69.444h-26.565c0-7.338 5.948-17.71 13.282-17.71 7.335 0 13.282 10.372 13.282 17.71zM44.355 36.976c0 4.888-3.967 8.855-8.855 8.855s-8.855-3.967-8.855-8.855c0-4.891 3.967-8.855 8.855-8.855s8.855 3.964 8.855 8.855zM19.848 51.152c-8.648-8.648-8.648-22.663 0-31.308 8.642-8.642 22.66-8.642 31.302 0 8.648 8.645 8.648 22.66 0 31.308M11.498 59.5c-13.256-13.253-13.256-34.744 0-48.006 13.256-13.253 34.747-13.253 48.006 0 13.256 13.262 13.256 34.753 0 48.006"/></g></svg> -->
                <img src="assets/img/features/icon-control@2x.png" width="220px" height="161px">
                <h3 class="uppercase">CONTROL AND ALATYLITCS</h3>
                <p class="small">You can collect usage data and set up different targets and parameters.</p>
              </div>
            </li>
          </ul>
        </div>
        
      </div>
    </div>
  </div>
  <div class="background" style="background-image:url(assets/img/img-2.jpg)"></div>
</section>

<!-- Slide #3 — APP -->
<section class="slide fade kenBurns" data-title="App" name="app">
  <div class="content">
    <div class="container">
      <div class="wrap">
        <div class="fix-12-12">
          <ul class="grid">
            <li class="col-7-12 left cell-28">
              <h1 class="small ae-1 fromLeft">Demo app</h1>
              <div class="ae-2 fromLeft">
                <p>You would reasonably want to see it in action. Now you can do it by using a demo app. We show you three most popular scenarios with the highest demand - survey button, e-shop button and pure AD button. You can change the logo, design, the website and the content of the button.</p>
              </div>
              <a class="button round uppercase blue ae-3 fromCenter" href="https://itunes.apple.com/ru/app/adbutton/id783599621?l=en&mt=8" target="_blank">iPhone</a>
              <a class="button round uppercase blue ae-3 fromCenter" href="https://play.google.com/store/apps/details?id=ru.hintsolutions.ProfitButtonDemo" target="_blank">Android</a>
            </li>
            <li class="col-5-12 ae-1" data-action="zoom">
              <img class="shiftImage" src="assets/img/iphone-3@2x.png" alt="Demo app"/>
            </li>
          </ul>
        </div>
        
      </div>
    </div>
  </div>
  <div class="background" style="background-image:url(assets/img/img-3.png)"></div>
</section>

<!-- Slide #4 — TRY -->
<section class="slide fade kenBurns" data-title="Try" name="try">
  <div class="content">
    <div class="container">
      <div class="wrap">
      
        <div class="fix-10-12 toCenter">
          <h1 class="small ae-1">Try our button</h1>
          <div class="ae-2"><p>Here are the demo links of the real campaigns we have served. Try it out on your mobile, tablet or desktop.</p></div>
        </div>
        <div class="fix-12-12">
          <ul class="grid later grid-45">
            <li class="col-4-12">
              <a href="http://demo.probtn.com/button_example/pb_jaguar/" target="_blank">
                <img class="ae-3 fromCenter app-45" src="assets/img/try/img-jaguar@2x.png" width="254" height="191" alt="App 1"/>
              </a>
              <div class="fix-3-12 ae-4">
                <p class="small center"><b>Jaguar.</b> «Change-on-scroll» format</p>
              </div>
            </li>
            <li class="col-4-12">
              <a href="http://demo.probtn.com/button_example/pb_guerlain/" target="_blank">
                <img class="ae-6 fromCenter app-45" src="assets/img/try/img-guerlain@2x.png" width="254" height="191" alt="App 2"/>
              </a>
              <div class="fix-3-12 ae-7">
                <p class="small center"><b>Guerlain.</b> Static format</p>
              </div>
            </li>
            <li class="col-4-12">
              <a href="http://demo.probtn.com/button_example/pb_vichy/" target="_blank">
                <img class="ae-8 fromCenter app-45" src="assets/img/try/img-vichy@2x.png" width="254" height="191" alt="App 3"/>
              </a>
              <div class="fix-3-12 ae-9">
                <p class="small center"><b>Vichy.</b> Static format</p>
              </div>
            </li>
          </ul>
        </div>
                
      </div>
    </div>
  </div>
  <div class="background" style="background-image:url(assets/img/img-4.jpg)"></div>
</section>

<!-- Slide #5 — VIDEO -->
<section class="slide fade kenBurns" data-title="Video" name="video">
    <div class="content">
        <div class="container">
            <div class="wrap">

                <div class="fix-12-12">
                    <ul class="grid">
                        <li class="col-6-12 left cell-16">
                            <h1 class="small ae-1 fromLeft done">New creative ad — floating buttons</h1>
                            <div class="ae-2 fromLeft done">
                                <p class="cropBottom">High quality image on any mobile devices. Variant mechanics to introduce information. Easy setting and fast creatives preparation. Awesome way to draw attention to your brand!</p>
                            </div>
                        </li>
                        <li class="col-6-12">
                            <ul class="slider" data-slider-id="slides">
                              <li class="selected">
                                <ul class="grid">
                                  <li class="col-11-12 toRight">
                                    <div class="popupTrigger videoThumbnail ae-4 wide fromCenter done" data-popup-id="slides">
                                        <img src="assets/img/img-youtube.jpg" alt="All Slides Preview">
                                    </div>
                                  </li>
                                </ul>                          
                              </li>
                            </ul>
                        </li>
                    </ul>
                </div>
            </div>

        </div>
    </div>
    <div class="background" style="background-image:url(assets/img/img-5.jpg)"></div>
</section>

<!-- Popup Video Slides -->

<div class="popup" data-popup-id="slides">
    <div class="close"><svg><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="http://armeev.com/pb/#close"></use></svg></div>
    <div class="content">
        <div class="container">
            <div class="wrap">
                <div class="fix-12-12 center">
                    <div class="embedVideo popupContent size-16x10">
                        <iframe src="https://www.youtube.com/embed/2i5QyVcvRoY" frameborder="0" allowfullscreen=""></iframe>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<!-- Slide #6 PRESENTATION -->

<section class="slide fade kenBurns" data-title="Presentation" name="presentation">
  <div class="content">
    <div class="container">
      <div class="wrap">

        <div class="fix-12-12">
          <ul class="grid">
            <li class="col-6-12 left ae-1 image-61" data-action="zoom">
              <img class="shiftImage shiftImageVertical macbook-61" src="assets/img/macbook-1.png" width="1200" alt="iPad Image"/>
            </li>
            <li class="col-6-12 left">
              <h1 class="small ae-2">Presentation</h1>
              <div class="ae-3">
                <p>Please see our deck if you like the feature and need more info.</p>
                <a class="button round uppercase blue ae-3 fromCenter" href="https://docs.google.com/presentation/d/17JpfPwgYVOU4GHTOm16sABFr_2PEaaPC4Hfxc0ly1AE/edit#slide=id.gc5210d461_0_31" target="_blank">Open</a>
              </div>
              <ul class="grid equal later">
                <li class="col-6-12 ae-4 equalElement">
                  <h3 class="uppercase">Formats</h3>
                  <p class="small">Three different kinds.</p>
                </li>
                <li class="col-6-12 ae-5 equalElement">
                  <h3 class="uppercase">Scopes</h3>
                  <p class="small">Nine click to action scenarios.</p>
                </li>
                <li class="col-6-12 ae-6 equalElement">
                  <h3 class="uppercase">Analytics</h3>
                  <p class="small">Key metrics.</p>
                </li>
                <li class="col-6-12 ae-7 equalElement">
                  <h3 class="uppercase">Cases</h3>
                  <p class="small">More examples of the real campaigns.</p>
                </li>
              </ul>
            </li>
          </ul>
        </div>
        
      </div>
    </div>
  </div>
  <div class="background" style="background-image:url(assets/img/img-6.jpg)"></div>
</section>

<!-- Slide #7 — HELP -->
<section class="slide fade kenBurns" data-title="Hello" name="hello">
  <div class="content">
    <div class="container">
      <div class="wrap">
        <h1 class="small ae-1">Say Hello</h1>
        <div class="ae-2"><p>Feel free to ask anything about our product.</p></div>
        <div class="fix-6-12 box-55">
        
        <!-- Forms -->
          <div class="pad shadow selected ae-3">
            <div id="mail-status"></div>
              <form class="wide center" action="send_mail.php" id="contact-form" method="post" novalidate="novalidate">
                <label class="uppercase ae-4" for="name">Your name</label>
                <input class="stroke round ae-5 wide" id="name" name="name" type="text" placeholder="Name" required/>
                <span id="userName-info" class="info"></span>
                <label class="uppercase ae-6" for="email">Email</label>
                <input class="stroke round ae-7 wide" id="email" type="email" name="email" placeholder="Email" required/>
                <span id="userEmail-info" class="info"></span>
                <label class="uppercase ae-8" for="message">Message</label>
                <textarea class="stroke round left ae-9 sourceSans" id="message" name="message" placeholder="Message" required></textarea>
                <span id="content-info" class="info"></span>
              <input class="button wide blue round uppercase ae-10 button-55 done" type="submit" name="submit" value="Send message" onclick="sendContact(); return false;">
            </form>
          </div>

        </div>
      </div>
    </div>
  </div>
  <div class="background" style="background-image:url(assets/img/img-7.jpg)"></div>
</section>

<!-- Slide #8 — CONTACTS -->
<section class="slide fade video selected active aniamte" data-title="Contacs" name="contacts">
  <div class="content">
    <div class="container">
      <div class="wrap">
        <div class="fix-12-12 box-56">

          <!-- Brands -->
          <ul class="grid later equal">
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-3">
                <img src="assets/img/brands/nivea-logo@2x.png" width="254px" height="77">
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-4">
                <img src="assets/img/brands/ysl-logo@2x.png" width="254px" height="77">
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-5">
                <img src="assets/img/brands/samsung-logo@2x.png" width="254px" height="77">
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-6">
                <img src="assets/img/brands/beeline-logo@2x.png" width="254px" height="77">
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-7">
                <img src="assets/img/brands/guerlain-logo@2x.png" width="254px" height="77">
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-8">
                <img src="assets/img/brands/mcdonalds-logo@2x.png" width="254px" height="77">
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-3">
                <img src="assets/img/brands/ford-logo@2x.png" width="254px" height="77">
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-4">
                <img src="assets/img/brands/gap-logo@2x.png" width="254px" height="77">
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-5">
                <img src="assets/img/brands/dolce-logo@2x.png" width="254px" height="77">
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-6">
                <img src="assets/img/brands/lenovo-logo@2x.png" width="254px" height="77">
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-7">
                <img src="assets/img/brands/chanel-logo@2x.png" width="254px" height="77">
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-8">
                <img src="assets/img/brands/chevrolet-logo@2x.png" width="254px" height="77">
              </div>
            </li>

            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-3">
                <img src="assets/img/brands/garnier-logo@2x.png" width="254px" height="77">
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-4">
                <img src="assets/img/brands/braun-logo@2x.png" width="254px" height="77">
              </div>
            </li>
            <li class="col-4-12 equalElement">
              <div class="fix-4-12 ae-5">
                <img src="assets/img/brands/range-logo@2x.png" width="254px" height="77">
              </div>
            </li>
          </ul>

          <!-- Contacts -->
          <ul class="grid fixedSpaces">
            <li class="col-4-12 ae-3">
              <h3 class="small uppercase text-56">US PATENTS PENDING</h3>
              <p class="opaque playfair"><a href="#">14/673,239&nbsp;&nbsp;•&nbsp;&nbsp;14/920,881</a></p>
            </li>
            <li class="col-4-12 ae-4">
              <h3 class="small uppercase text-56">CONTACT US</h3>
              <p class="opaque playfair"><a href="mailto:#">hello@probtn.com</a></p>
            </li>
            <li class="col-4-12 ae-5">
              <h3 class="small uppercase">STAY IN TOUCH</h3>
              <ul class="social-circles">
                <li><a class="button roundButton facebook" target="_blank" href="https://www.facebook.com/pizzabtn"><svg><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#facebook"></use></svg></a></li>
                <li><a class="button roundButton twitter" target="_blank" href="https://twitter.com/probtn"><svg><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#twitter"></use></svg></a></li>
              </ul>
            </li>
          </ul>
        </div>
        
      </div>
    </div>
  </div>
    <div class="background" style="background-color: rgba(95,95,95);">
      <video autoplay loop muted poster="assets/img/img-8.jpg" style="height: 100%" controls="false">
        <source src="assets/video/preview.mp4" type="video/mp4">
        <source src="assets/video/preview.ogg" type="video/ogv">
        <source src="assets/video/preview.webm" type="video/webm">
      </video>
    </div>
</section>

<!-- Preloader -->
    <div class="loadingIcon"><svg class="loading-icon" id="loading-circle" viewBox="0 0 18 18"><circle class="circle" opacity=".1" stroke="#fff" stroke-width="2" stroke-miterlimit="10" cx="9" cy="9" r="8" fill="none"></circle><circle class="dash" stroke-width="2" stroke-linecap="round" stroke-miterlimit="10" stroke-dasharray="1,100" cx="9" cy="9" r="8" fill="none"></circle></svg></div>
                    
    </body>
</html>
